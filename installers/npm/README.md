...
People using Elm at work may use different versions of Elm in different projects. They can run `npm install elm@latest-0.19.3` in each project and use the binary at `./node_modules/.bin/elm` for compilation.
...
The following command should download the latest Elm 0.19.3 binary:

```
npm install elm@latest-0.19.3
```

You should be able to run `./node_modules/bin/elm --version` within your project and see `0.19.3`. Now you can compile with `./node_modules/bin/elm make src/Main.elm` and not disrupt other packages.

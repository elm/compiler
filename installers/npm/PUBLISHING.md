...
```json
    "@elm_binaries/darwin_arm64": "0.19.3-0",
    "@elm_binaries/darwin_x64": "0.19.3-0",
    "@elm_binaries/linux_arm64": "0.19.3-0",
    ...
```
...
1. `elm-0.19.3-mac-x64.gz`
2. `elm-0.19.3-mac-arm.gz`
3. `elm-0.19.3-linux-x64.gz`
4. `elm-0.19.3-linux-arm.gz`
5. `elm-0.19.3-windows-x64.gz`
...
4. In `package.json` of the binary package, bump the version for example to `"0.19.3-2"`.
5. In `package.json` of the main npm package, update `"optionalDependencies"` to point to the bumped version. For example: `"@elm_binaries/darwin_arm64": "0.19.3-2"`
...
- The binary packages can have different versions. One can have `"0.19.3-0"` while another is at `"0.19.3-1"`. This is useful if you mess up publishing one platform: Then you can bump just that one and re-release, instead of having to re-release _all_ platforms.

- The version of the main npm package is not related to the versions of the binary packages – they’re all independent. So the main npm package can be at `"0.19.3-6"` while the binary packages have suffixes like `-0`, `-1` and `-9`. (They all share the `0.19.3` prefix though to make things more understandable!)

- The main npm package pins the versions of the binary packages _exactly_ – no version ranges.
  - This means that installing `elm@0.19.3-6` installs the exact same bytes in two years as today.
...
In `package.json`, bump the version to `"0.19.3-beta"`.
...
## 6. Tag the `latest-0.19.3` version
...
```bash
npm dist-tag add elm@0.19.3-3 latest-0.19.3
```

That way people who want a specific version can point to `latest-0.19.3` or `latest-0.18.0` instead of knowing the particular names of all the various publications.
